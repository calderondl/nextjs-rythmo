                    ...
                    <div className='flex justify-center'>
                        <div className='mt-6 p-2'>
                            <div className='flex rounded overflow-hidden shadow-lg'>
                                <img className='w-44 h-44' src={user && user.images[0] ? user.images[0].url : null} alt='User path' />
                                <div className='w-80'>
                                    {
                                        !athlete ? (
                                            <>
                                                <h1 className='font-bold text-2xl mb-2'>{user ? user.display_name : null}</h1>
                                                <p className='text-justify pl-3 pr-3'>Su usuario no está registrado en la base de datos de El Gimnasio. Por favor, acercarse a atención a cliente y solicitar que lo registren.</p>
                                                <div className='mt-4'>
                                                    <span className='font-extrabold text-md'>Atleta inactivo</span>
                                                </div>
                                            </>
                                        ) : (
                                            <>
                                                <h1 className='font-bold text-2xl mb-3'>{athlete ? athlete.nombre + ' ' + athlete.apellido : null}</h1>
                                                <p className='text-left pl-3'>correo: {athlete ? athlete.correo : null}</p>
                                                <p className='text-left pl-3'>telefono: {athlete ? athlete.telefono : null}</p>
                                                <div className='mt-4'>
                                                    <span className='font-extrabold text-md'>Atleta activo</span>
                                                </div>
                                            </>
                                        )
                                    }
                            ...
                            <div className='mt-6'>
                                ...
                                <button onClick={() => { logOff(athlete) }} className='bg-red-500 rounded-full font-bold text-white p-4 hover:bg-red-600'>CERRAR SESIÓN</button>
                            </div>
                        </div>
                    </div>
                )
