    ...
    useEffect(() => {
        ...
        if (_token) {
            ...
            spotifyInstance.getMe().then(_user => {
                setUser(_user)
                console.log(_user)
                getAthlete(_user, _token)
            })
        }
    }, []);

    async function getAthlete(_user, _token) {
        await axios.post('api/getAthlete', {
            ...
            .then(function (response) {
                ...
                if (_athlete) {
                    setAthlete(_athlete)
                    addOnLine(_athlete, _token)
                }
            })
    }

    async function addOnLine(_athlete, _token) {
        await axios.post('api/addOnLine', {            
            spotifyid: _athlete.spotifyid,
            token: _token
        })
    }
